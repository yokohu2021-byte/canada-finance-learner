...
import { categories, isLocale, siteCopy, type Locale } from "../../lib/i18n";
    ...
    <div>
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-24">
        <div className="max-w-3xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href={`/${locale}/free-consultation`} className="rounded-full bg-brand px-6 py-3 font-semibold text-white hover:bg-blue-700">
              {copy.hero.cta}
            </Link>
            <Link href={`/${locale}/articles`} className="rounded-full bg-white px-6 py-3 font-semibold text-ink ring-1 ring-slate-200 hover:bg-slate-50">
              {copy.hero.secondary}
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div className="mb-6 flex items-end justify-between gap-4">
          ...
          <Link href={`/${locale}/articles`} className="text-sm font-semibold text-brand hover:text-blue-700">
            {copy.nav.articles} →
          </Link>
        </div>
